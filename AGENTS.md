# MarketDashboard - V10.2 Agent Instructions

二级市场投资辅助工具，运行于 Codex/Claude 桌面环境，每天 4 次生成并通过 Gmail SMTP 推送「哈利每日 Market Brief」。

## 当前版本

**V10.2：公开 RSS 基础摘要 + 订阅外媒 Chrome 条件深读 + Futu 兜底。**

行情、Futu 新闻和 Bloomberg/FT/WSJ/NYT/Washington Post 公开 RSS 候选由 `scripts/fetch_data.py` 生成结构化 JSON；agent 只对高价值文章通过已登录 Chrome 深读原站正文。
...
| 机构研报 | Futu `news_type=3`，按自选股 → Mag7 → AI/半导体/新能源车等相关行业 → 市场热点股兜底 | 为空则用读者可见话术说明 |
| 外媒基础信息 | Bloomberg / FT / WSJ / NYT / Washington Post 公开 RSS（仅近期有效 feed 动态参与） | Futu News |
...
`scripts/fetch_data.py` 已实现：多查询召回、7 天历史去重、跨板块去重、48h/7d 时效窗口、标题相似度去重、个股短名陷阱过滤、来源加权、时间排序、研报过滤、研报优先级排序、社区近 72h 过滤、社区关键词扩展和美股热点发现。
...
- 美股热点只进入 `brief-us-preopen` 和 `brief-us-close`；`hot_sectors` 为空只省略板块小段，领涨/领跌/最活跃照常写，三者全空才整节省略。
- 研报和社区按模板写实际可用内容；数量不足时不得暴露返回数量、字段名或补写空泛判断。
- 「深度研报 / 机构观点」选股优先级固定为：自选股 > Mag7 > AI/半导体/新能源车等相关行业 > 市场热点股兜底；不得一上来用热点股拼成信息大杂烩。脚本按优先级轮转选条，同一公司最多 2 条，写作时也不要把整栏写成同一家公司。
...
2. **宏观 / 央行 / 大宗**：至少 3 条新闻信息；至少 1 条外媒 + 至少 1 条 Futu。
3. **AI / 大模型 / 芯片**：至少 3 条新闻信息；至少 1 条外媒 + 至少 1 条 Futu。
4. **个股 / 行业聚焦**：1-2 条外媒 + 至少 1 条 Futu。

正常使用 7-10 条外媒信息，至少覆盖 2 家；有合适内容时尽量覆盖更多来源，但不为凑数量牺牲相关性。国际/宏观优先 48h，AI/个股趋势类最多 7 天。

个股选择优先级：自选股/显著异动 > 业绩与指引 > 并购/资本开支 > 行业结构变化。

写法固定采用最新测试邮件风格：标题短，摘要 2-3 句，第一句交代事实，后续句把外媒事实、Futu 事实和行情交叉验证。同一事件有多个来源时，必须优先合并成一条新闻并在来源行并列展示；不得拆成多条凑数量。

### 去重

- 同一事件跨媒体、跨板块只保留一条；可把外媒与 Futu 合并为多来源新闻，这是优先写法。
...
「深度研报 / 机构观点」和「社区观察」不得写成执行日志；数据不足时只用读者可见话术或省略空内容。研报栏必须保持针对性，按自选股、Mag7、相关行业、市场热点股的顺序选取。
...
- 单来源：`[FT](url) | MM-DD`、`[NYT](url) | MM-DD`
